...
export class Media implements MediaMetadata {
    ...
    coverArt: string = '';
    tree: GroupTile;
    readonly globalModulator: GlobalModulator;

    constructor(data: MediaMetadata, root?: GroupTile, globalModulator?: GlobalModulator) {
        for (const key in data) {
            (this as any)[key] = (data as any)[key];
        }
        this.tree = root ?? new GroupTile();
        this.globalModulator = globalModulator ?? createGlobalModulator();
    }

    /**
     * Deflate the Media into a compressed Tile layout.
     * @param consume Ignores data preservation and renders the layout unusable by
     * consuming audio buffers during compression and destroying modulators to free resources.
    ...
    async compress(consume: boolean = false): Promise<ArrayBuffer | null> {
        ...
        try {
            ...
            pushModulations(this.globalModulator, 'global');
            ...
            while (stack2.length > 0) {
                ...
                if ('modulator' in curr) {
                    ...
                    pushModulations(source, curr.id);
                }
    ...
    private static async decompressV2(data: MediaSchema.SchemaV2): Promise<Media> {
        ...
        const globalModulator = createGlobalModulator();
        for (const { source, target, transforms } of modulations) {
            // large breaking changes can bork old layouts
            if (target.id == 'global') throw new TypeError('Somehow your layout has global modulation targets before they\'ve been added');
            const modulator = source.id == 'global' ? globalModulator as Modulation.Source<any> : (() => {
                const tile = idToTiles.get(source.id);
                if (tile === undefined || !('modulator' in tile)) {
                ...
                return (tile as Modulation.Modulator<any>).modulator;
            })();
            if (modulator === undefined) continue;
            const modulation = (() => {
                const tile = idToTiles.get(target.id);
                if (tile === undefined || !('modulation' in tile)) {
                ...
                return (tile as Modulation.Modulatable<any>).modulation;
            })();
        ...
        // media
        return new Media(metadata, root, globalModulator);
    }

    /**
     * Frees resources held by the tree and modulators. **Never calling this will create resource leaks!**
     */
    destroy() {
        this.tree.destroy();
